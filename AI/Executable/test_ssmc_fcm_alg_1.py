...
test_dataframe = dataframe[dataframe['battery_order'].isin(test_battery_orders)]

for i in range(0, 9):
    y_test = dataframe[dataframe['battery_order'] == test_battery_orders[i]][["Qi"]]
    y_test = y_test.to_numpy()
    cycles = range(0, len(y_test))
    s_idx = test_dataframe[dataframe['battery_order'] == test_battery_orders[i]].index[0]
    e_idx = test_dataframe[dataframe['battery_order'] == test_battery_orders[i]].index[-1]
    y_pred = pred_Qi[s_idx:e_idx + 1]
